...
if (isset($_POST['titolo'])) {
    ...
    // VULNERABILE
    $sql = "UPDATE documenti_segreti SET titolo = '$titolo', contenuto = '$contenuto', livello_riservatezza = '$livello' WHERE id = $id";
    $conn->multi_query($sql);
    header("Location: documenti_segreti.php?msg=Documento Aggiornato");
    exit();
}

$res = $conn->query("SELECT * FROM documenti_segreti WHERE id = $id");
if (!$res || $res->num_rows == 0) die("Documento non trovato.");
$doc = $res->fetch_assoc();
?>
<!DOCTYPE html>
<html lang="it">
<head>
    <meta charset="UTF-8">
    <title>Modifica Documento | Admin</title>
    <style>
        ...
        .id-badge { background-color: #fee2e2; color: #991b1b; padding: 4px 10px; border-radius: 20px; font-size: 16px; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="container">
        ...
        <h2>Modifica Documento <span class='id-badge'>#<?php echo $id; ?></span></h2>
        
        <form method="POST">
            <div class="form-group">
                <label>Titolo Documento:</label>
                <input type="text" name="titolo" value="<?php echo htmlspecialchars($doc['titolo']); ?>" required>
            </div>
            <div class="form-group">
                <label>Livello di Riservatezza:</label>
                <select name="livello_riservatezza" required>
                    <option value="RISERVATO" <?php if($doc['livello_riservatezza']=='RISERVATO') echo 'selected'; ?>>RISERVATO</option>
                    <option value="CONFIDENZIALE" <?php if($doc['livello_riservatezza']=='CONFIDENZIALE') echo 'selected'; ?>>CONFIDENZIALE</option>
                    <option value="TOP SECRET" <?php if($doc['livello_riservatezza']=='TOP SECRET') echo 'selected'; ?>>TOP SECRET</option>
                </select>
            </div>
            <div class="form-group">
                <label>Contenuto Sensibile:</label>
                <textarea name="contenuto" required><?php echo htmlspecialchars($doc['contenuto']); ?></textarea>
            </div>
